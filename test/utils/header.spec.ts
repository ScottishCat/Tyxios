import { parseHeaders, setHeaders, flatenHeaders } from '../../src/utils/header'
...
describe('helpers:header', () => {
  ...
  describe('processHeaders', () => {
    ...
    test('should set not Content-Type if not set and data is not PlainObject', () => {
      const headers: any = {}
      setHeaders(headers, new URLSearchParams('a=b'))
      expect(headers['Content-Type']).toBeUndefined()
    })
  ...
  describe('flattenHeaders', () => {
    test('should flatten the headers and include common headers', () => {
      ...
      expect(flatenHeaders(headers, 'get')).toEqual({
        Accept: 'application/json',
        'X-COMMON-HEADER': 'commonHeaderValue',
        'X-GET-HEADER': 'getHeaderValue'
      })
    ...
    test('should flatten the headers without common headers', () => {
      ...
      expect(flatenHeaders(headers, 'patch')).toEqual({
        Accept: 'application/json'
      })
    })

    test('should do nothing if headers is undefined or null', () => {
      expect(flatenHeaders(undefined, 'get')).toBeUndefined()
      expect(flatenHeaders(null, 'post')).toBeNull()
    })
  })
})